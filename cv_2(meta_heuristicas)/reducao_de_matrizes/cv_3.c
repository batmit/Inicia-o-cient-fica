...
int main() {
    ...
    branchAndBound(
        vetorCidades, 1, caminho, 0,
        matrizDistancia, &melhorCusto,
        resultadoFinal, pool, limiteRaiz,
        vizinhosOrdenados
    );
...
void branchAndBound(
    ...
    if (nivel == CIDADES) {
        double custoTotal = custoAtual
                         + matrizDistancia[caminho[nivel - 1]][caminho[0]];
    ...
    for (int i = 0; i < CIDADES - 1; i++) {
        ...
        if (vetorCidades[candidato].visitado == 0) {
            ...
            for (int r = 0; r < CIDADES; r++)
                memcpy(pool[nivel][r], pool[nivel - 1][r],
                       CIDADES * sizeof(double));
            ...
            // Bloqueia retorno prematuro à origem (evita subciclo)
            ...
            double novoCusto = custoAtual
                             + matrizDistancia[ultimaCidade][candidato];
            ...
            if (novoLimite < *melhorCusto) {
                ...
                branchAndBound(
                    vetorCidades, nivel + 1, caminho,
                    novoCusto, matrizDistancia,
                    melhorCusto, resultadoFinal,
                    pool, novoLimite,
                    vizinhosOrdenados
                );
...
void ordenarVizinhos(int **vizinhosOrdenados, Cidade *vetorCidades,
...
double vizinhoMaisProximo(Cidade *vetorCidades, double **matrizDistancia,
                          int *resultadoFinal, int posInicial) {
    double soma = 0;
...
double multiStart(Cidade *vetorCidades, double **matrizDistancia,
                  int *resultadoFinal) {
    ...
    for (int i = 1; i < CIDADES; i++) {
        ...
        for (int j = 1; j < CIDADES; j++)
            if (j != i) vetorCidades[j].visitado = 0;
        ...
        double custo;
        int *melhorRota;
        if (vmP < insercao) { custo = vmP;     melhorRota = rotaAtual; }
        else                 { custo = insercao; melhorRota = vetAux;   }
    ...
    while (melhorou) {
        ...
        for (int i = 1; i < CIDADES - 1; i++) {
            for (int j = i + 1; j < CIDADES; j++) {
                ...
                if (matrizDistancia[a][c] + matrizDistancia[b][d] <
                    matrizDistancia[a][b] + matrizDistancia[c][d]) {
        ...
        for (int k = 0; k < CIDADES; k++) {
            if (vetorCidades[k].visitado == 0) {
                for (int pos = 0; pos < tamCiclo; pos++) {
                    ...
                    int cj = (pos == tamCiclo - 1)
                             ? resultadoFinal[0]
                             : resultadoFinal[pos + 1];

                    double delta = matrizDistancia[ci][k]
                                 + matrizDistancia[k][cj]
                                 - matrizDistancia[ci][cj];
